...
void eInit(string filename)
{
    ...
    P.T.row.resize(0);
    P.editmode_ = 0;
    getWindowSize(&P.limitrows_, &P.limitcols_);
    P.limitrows_ -= 2;
    eReadFile();
}
/*Terminal*/
...
int enableEditorTerminal()
{
    ...
    termios_Editor.c_cc[VMIN] = 0;  /* Return each byte, or zero for timeout. */
    termios_Editor.c_cc[VTIME] = 1; /* 100 ms timeout (unit is tens of second). */
